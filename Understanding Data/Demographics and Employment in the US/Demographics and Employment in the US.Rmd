...
**Answer**: From the summary function above the most common industry is "Educational and health services".
...
**Answer**: The Married variable being missing is related to the Age value.
...
How many observations (codes for metropolitan areas) are there in MetroAreaMap? How many observations (codes for countries) are there in CountryMap?
```{r}
...
```{r}
sort(tapply(CPS$Race=="Asian", CPS$MetroArea, mean),decreasing=TRUE)[1:10]
```
...
**Answer**: From the summary function above we see the most common POB outside North America is the Philippines.
